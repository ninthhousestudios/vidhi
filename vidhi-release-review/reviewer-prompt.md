...
A fixed baseline of Fowler code smells (_Refactoring_, ch.3). Two rules bind it:
...
- **Always a judgement call.** Report each as a labelled heuristic ("possible Feature Envy"), never a hard violation — `category: design`. Skip anything tooling (clippy/analyzer, sutra constraints, the pack's code-intel) already enforces or reports — e.g. sutra's duplicate detection covers Duplicated Code; add a smell finding only where you have something the tool didn't say.
...
- **Repeated Switches** — the same `switch`/`if`-cascade on the same type recurs across the codebase. → replace with polymorphism, or one map all sites share.
- **Shotgun Surgery** — one logical change forces scattered edits across many files. → gather what changes together into one module.
- **Divergent Change** — one file or module is edited for several unrelated reasons. → split so each module changes for one reason.
- **Speculative Generality** — abstraction, parameters, or hooks added for needs nothing has. → delete it; inline back until a real need shows.
