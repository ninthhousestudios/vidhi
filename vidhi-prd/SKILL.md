...
3. Write the PRD using the template below, then publish it to the project issue tracker. Apply the `needs-triage` triage label so it enters the normal triage flow. If the source task (brainstorm, request, etc.) belongs to a yojana arc, publish the PRD to the same arc with `arc_phase: prd`. If it does not belong a yojana arc, publishing to an arc it not needed, unless requested by the user
